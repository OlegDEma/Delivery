...
export async function GET(request: NextRequest) {
  ...
  const headers = [
    ...
    'Вартість доставки', 'Страхування', 'Пакування (€)',
    'Адресна доставка', 'Пункт збору', 'Пакет (сума)', 'Пакет (комісія)',
    'Загальна вартість',
    'Платник', 'Оплата', 'Сплачено',
    'Прийняв (кур\'єр)',
    'Дата створення',
  ];
  ...
  parcels.forEach((p, i) => {
    ws.addRow([
      ...
      p.insuranceCost ? Number(p.insuranceCost) : '',
      p.packagingCost ? Number(p.packagingCost) : '',
      p.addressDeliveryCost ? Number(p.addressDeliveryCost) : '',
      p.pickupPointCost ? Number(p.pickupPointCost) : '',
      p.parcelMoneyAmount ? Number(p.parcelMoneyAmount) : '',
      p.parcelMoneyCost ? Number(p.parcelMoneyCost) : '',
